...
```{r, echo = FALSE, dependson="load_data"}
...
```{r library1, message = FALSE, warning = FALSE, exercise = TRUE, dependson="load_data"}
library(sna)
betweenness(friendship)
```

```{r, message = FALSE, warning = FALSE, echo = FALSE}
try(library(sna), silent = TRUE)
```

Nice, and if you wanted to see the names of the actors next to their scores, you 
could run:

```{r library1a, exercise = TRUE, dependson="load_data"}
...
```{r influenceR_laden, exercise = TRUE}
library(influenceR)
```
...
```{r library_error, exercise = TRUE, dependson =c("load_data", "influenceR_laden")}
...(friendship)
```
...
package throws an error ("<code>Not a graph object</code>"), because it expects 
an object in a different form than
the object `g`. But if `influenceR` would have been able to directly deal with
this object `friendship`, it would not give an error and might have given you a
very different result than you wanted (because who says that the `betweenness`
function does the exact same thing in both the `influenceR` and the `sna`
packages?).

Now, first predict what will happen when you run the follow code, and then
indeed run it. Does it do what you expected?

```{r library2a, exercise = TRUE, dependson="load_data"}
...
```{r library3, exercise = TRUE, dependson="load_data"}
library(sna)
