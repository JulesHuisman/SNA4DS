---
title: "Lab 07 - Stats III - ERGMs - Part two"
output: 
  learnr::tutorial:
    fig_caption: no
    progressive: true
...
However, very often networks are larger than the Florentine or the Sampson's ones and the relationships they map down require more sophisticated efforts to be 
explained.

Let's take a look to the `faux.mesa.high` data set, which is one of the networks
included in the `ergm` package and employed in the `statnet` tutorials. Remember,
`statnet` is the project that developed the `r` package `ergm`, and their tutorials are the first source of information if you are running ERGMs in `r`.
...
The mesa network contains information about 203 relationships among 205 students 
enrolled at the Mesa High School in the US. Well, not properly the real information about the students since it is classified. Something close enough, that allow us to explore the network without revealing any sensitive data. 
...
Carefully observe the nature of the relationships in the plots. Do you think 
that friendship can be driven by Grade, Race or Sex? That's the research question we are going to address!
...
Let's focus on Grade. This nodal attribute is categorical, hence we need to 
choose terms that can handle this typology of data. 

Since we want to know whether being in the same grade fosters the formation of 
friendship we can use the term `nodematch`. This term handles categorical data 
and explores the existence of homophilic relationships among people that share the same category (grade in this case).

Another term that we can use with a categorical nodal attribute is`nodefactor`.
This terms outputs one statistic for each category in the attribute minus one 
to avoid linear dependency. Each of these statistics gives the number of times 
a node with that attribute and that category appears in an edge in the network.
...
Every statistic in the result's table allow us to reject the null hypothesis of
no effects. The coefficients produced by the `nodematch` term inform us on the 
existence of homophilic effects in each grade. Grade 12 is the one where these 
effect are less likely to happen. 

The `nodefactor` term inform us on the probability that nodes that are connected
to other nodes are characterized by a certain grade. We can say that students in 
grade 12 are more likely to have friends than students in the other grades.

What happens if we try to run the same exploratory model, but focusing on race? Can you do it? Call the model 'mesaM2' and give it a go. Don't forget to print the summary, we are interested on the results!
...
We can see that the two categories that produced the `-Inf` coefficients 
correspond to a 0 in the descriptive statistics's table. Does it mean that 
we don't have any student of Race black and other in the school? 
Let's check the frequency of the nodal attribute to dig deeper

```{r mesaErgmRaceFreq, exercise = TRUE, exercise.setup = "load_mesa"}
table(network::get.vertex.attribute(mesa, "Race"))
```
Nope! There are no empty categories in the variable, the zero corresponds to the 
homophilic cases measured by the term's formula. 
...
The zeros in the descriptive statistics with the model specifications tell us that there are no homophily effects for those two categories, but these effects are still in place for the other categories. 

It's always crucial to check both the nodal attribute's frequency and the 
descriptive statistics of model specification. Only checking both we can have 
the full picture and avoid mistakes such as treating this case as a missing data 
one. 
...
It indicates that there is linear dependence between the results of these two terms. `nodematch` tell us whether there are homophilic effects due to sex, while `nodefactor` tell us what's the probability for female and male students to make friends. In this context homophily is more interesting, hence since we need to get rid of one term to avoid linear dependence, let's discard `nodefactor`. 

Please not that in this exploratory model I used the levels argument to tell the software to compute and print the statistic for the female category rather than the male one. That's how you can customize your output with this term. The options to do so might be different from term to term, hence you need to rid the manual. 
...
When it comes to Race the term looking for homophily shows two empy categories. What do we do?
...
For what concerns Race in the mesa school, since some categories are empty, but 
others are not, removing the term might seem a loss of information. We can 
then specify the more generic option that assesses whether there is homophily 
overall, not informing us about each category, but providing one single statistic
for the all network. 

For what concern the Sex attribute, let's keep differential homophily and remove the `nodefactor` term.
...
This looks like a pretty nice mode! You know already how to understand significance and coefficients, so there is no need to comment on that, but let's comment on the overall meaning on this model. 

* nodematch-grade (differential homophiliy): There is an homophily effect due to 
grade. The strongest homophily is in grade 7 and the weakest in grade 12.

* nodefactor-grade: this terms tell us the probability of students in each grade 
to make friends. Grade 12 students have the highest number of friends.
...
female students, but not among male.
...
So far we explored only dyadic independent terms that look at the probability of relationship has a consequence of properties of the nodes. We looked at nodal attributes, or the simple node propensity to connect to others (`edges`, `sender`, `receiver`). 

We already know that there is more than this. 

With these dyadic independent model configurations we cannot explore situations where you meet a new friend that was in school with your roommate, or we cannot explore situations where you start to follow someone on Instagram just because all your other friends do, among other examples. 
...
This text inform us on the fact that the results are estimated computing maximum
pseudolikelihood estimation (MPLE). 

Let's check a dyadic dependent example to compare. We can simply add the dyadic
dependent term `degree` to the previous model. With this specification it
checks on the probability that the number of nodes with degree one in the florentine business network is non-random. We care only about the output text now though. Let's read on
...
```
This text tell us that results are computed using Monte Carlo Markov Chains 
(MCMC). What does it mean? 
Dyadic dependent models cannot be estimated as the dyadic independent ones. 
They are solved using Monte Carlo Markov Chains, that stochastically approximate
...
Nitty gritty: If you get great statistics next to poor results in the diagnostics you can trash your model. It's important.  

## MCMC Dyagnostics

While solving an ERGM with MPLE is almost the same as running Logit Models, computing your results using MCMC simulations is something quite far from that. Conceptually you are simulating (Monte Carlo) a large number of networks (Markov chains) that are described and evolve according to the terms that you specified. The values assumed by each parameter in the simulation are 'managed' by an algorithm (such as the Metropolis Hastings) that helps them to get closer to the observed network's values. If the estimates do not progressively get closer to the observed network's ones, it means that your model is not working as it should and you get a poor fit, or in the worse case scenario a degeneracy of the model. The latter indicates that the estimates do not 'converge' toward the values in the observed network.

If the terms chosen for your model were important for your dyadic independent model, they are absolutely crucial for your dyadic dependent one since they are literally the rules that govern the world you are simulating. Your goal is to create a synthetic reality that reproduce the social processes that generated your observed network. If you specify the model in a different direction, your synthetic reality will look very different from the observed one. Same as in a dystopian movie, where some 'wrong specification' generates unwanted consequences. 

Let's use a metaphor. You go to a fancy shop and you buy the best cookie of your life. But it is too expensive! you can't possibly buy that cookie every week. Hence you decide to try to recreate the recipe at home making several attempts. 100 g of flour, 120 of sugar, 2 eggs? or 120 of flour, 100 of sugar and 1 egg? the You try all the combination that are more likely to taste like the original, until you approximate the original cookie. Let's also say that you have a little elf that helps you a little with the job. That's what the simulations and your specifications are doing. 

Let's take a look to what a good fit looks like! 
...
In the default function the `center` parameter is set to TRUE, meaning that the 0 represents the statistics from the observed network. 

The trace plots on the left hand side represents all the values that each parameter assumes during the simulation.

The density plot on the right hand side represents the distribution of that parameter during that simulation. 

They both need to 'develop' around the zero. The trace plot should look as regular as possible resembling a [fuzzy caterpillar](https://www.google.com/search?q=fuzzy+caterpillar&rlz=1C1GCEU_enNL916NL916&sxsrf=ALeKk02yI8htk3IAE7DL9w21ezfosrjDnQ:1604060591472&source=lnms&tbm=isch&sa=X&ved=2ahUKEwiUo4Kup9zsAhUwMewKHU_sArwQ_AUoAXoECBsQAw&biw=1536&bih=698&dpr=1.25).

The density plot should look like a bell curve. 


In this example we are examining the results are good and we comment on the trace plots saying that 'the chains mixed well'. 
The density plot of the `degree` term is not exactly a bell curve, but considering that this is a small sample, it is still close enough to a normal distribution.
...
In if you do observe this, your model is not working as it should and you are not simulating reality correctly.

Take into account that the second issue can be addressed increasing the number of simulated network, and increasing the burn-in. The burn-in tells the simulation to ignore the first chains (the left hand side of the trace plot) since it might take a while to the simulation to reach a stable state.  
...
2. You are using some terms that are well known for creating some convergence issues, such as `triangle`. If this is the case you should use their geometrically weighted equivalent, that are term that represent the same effects, which formula has been mathematically improved to address convergence issues. 

Let's see an example of non-convergence due to the second case, the employment of the `triangle` term. In both cases you observe the same signs of model degeneracy.
Hence, if you are not using any of the incriminated terms, you have poor theory.
...
We do it adding one more specification to the model:
...
Sometimes it also happens that your model does not degenerate and converges providing you with results, but if you check the mcmc.diagnostics you see that your model is not fitting very well your data. 
...
`gof` calculates p-values for geodesic distance, degree, and reachability summaries to diagnose the goodness-of-fit of exponential family random graph models. This time you want your p-value to be as high as possible (1). In fact, this time the p-value compares your observed data to simulated data and if you did a good job specifying your model the statistics that these two samples produce should be almost the same. 


Let's fit again our model with a nice MCMC diagnostic and check whether it also has a nice goodness of fit.
...
The plot simply visualizes these results using box plots. The blue square is the mean value and the black tick line shows the observed values. What we want is the black tick line to be as close as possible to the blue square and possibly never outside of the min and max interval. 

Running `gof` for degree only, also prints out the gof for the model statistics. It works exactly the same way and it shows how the statistics of the observed network performs in comparison to the simulated one. 

You interpret the other two remaining point analogously. 

How did this model do? While the model statistics are doing really good, it's not the case for degree. For what concerns low degrees the simulation is different from the observed data. However, it gets much better for higher degrees. The same happens with edgewise shared partners, while the geodesic distance does quite well for every value. 
...
So far we used only two dyadic dependent terms, `mutual` and `degree`. There are many of them, and we cannot possibly cover every single one in this tutorial. However, it is certainly worth to spent some time talking about triads, cycles and triangles since they are at the core of the idea of dyadic dependence since it checks on the formation of ties due to existing ties.

### The Triad Census

Do you remember the [triad census](https://www.sciencedirect.com/science/article/pii/S0378873310000158?casa_token=FqY1CGVPJaAAAAAA:zVltTjQfyFhczdDIcwUQJmZ3zJZFGDuyrvQ0TMog7LNOOK_wADA-r46-q9bD7x7HJ2KBXFD7) categorization introduced by Davis and Leinhardt (1972) that we talked about in class a few weeks ago? You should have understood that checking on triads is very important in network analysis and there are several `ergm` terms to help you on this job. 

Let's take a look to the term `triadcensus`

```{r triadcensus, exercise = TRUE}
ergm::search.ergmTerms(name = 'triadcensus')
```
If you use this term in an ERGM with a direct network, it is going to check on the existence of each of the 16 options and provide you with the probability of observing each of them. If you use the undirected one, it is going to check only on the configurations that make sense on undirected data. Can you tell how many are those?
...
`intransitive` checks on three of them: 021C, 201, 111D. Let's try to run it and to see whether this time our model might make more sense. But before that, why don't you check what the term `intransitive`exactly does and whether this might make sense knowing the features of the Sampson network?
...
summary(samM2)
```
This time our results seems to be more interesting. Obviously we cannot be sure since we did not check the MCMC diagnostics and the goodness of fit. If the model fit is good, what do these results mean for the Sampson network? 
...
When you want to analyze three nodes patterns, other terms that my be often useful are
...
For instance in the code below we are checking on four nodes cycles in the Sampson data. Why don't you try to think which number might make more sense for the Sampson data and re-run the model? Just try to analyze what you know about this network already and plot it below if it helps you remembering it. 
...
Let's create an undirected network with 16 nodes and density 0.1. 
Then let's say that we want to simulate two networks that when inserted into an ERGM model with the term `edges` will output a coefficient of -1.8 and with the term `kstar` set to two, will output a coefficient of 0.03. That's how you do it!
...
At this point we covered together all the crucial topics that you need to know in order to be able to run exponential random graph models. Do you still feel a bit shaky at the idea of running these models? 
...
For this task we are going to use the UK faculty data from the `igraphdata` package. But still, let's pretend that you spent one year of your time collecting this data about these British academics and that now you are looking forward to understand a bit better what is going on with your sample.
...
And we were right! It seems that there is something interesting here. Do you want to try to run it again using `mutual` instead?

Let's move on adding one or more terms to test whether there are groups of friends in the network. How many friends do we expect to find? let's try with `triangles` and take into account that this model might take a while.
...
Let's focus on our third hypothesis. We can do that checking for homophily considering the group a nodal attribute. We want to check on the probability of having homophilic effects in each group, hence we should check for differential homophily. 
...
It seems we were right in a certain way, but group four has a odd result! We should have checked the model descriptive statics before! Shame on us! ;)
