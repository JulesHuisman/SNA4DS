...
# make the matrix not symmetrical
mat[1,3] <- 1
...
```{r degree_directed, echo = TRUE, include = TRUE, exercise = TRUE}

# out degree
igraph::degree(net, v = V(net), mode = "out", loops = TRUE, normalized = FALSE)
# in degree
igraph::degree(net, v = V(net), mode = "in", loops = TRUE, normalized = FALSE)
# total degree
igraph::degree(net, v = V(net), mode = "total", loops = TRUE, normalized = FALSE)
...
```{r degree_undirected, echo = TRUE, include = TRUE, exercise = TRUE}
Unet <- as.undirected(net)

igraph::degree(Unet, v = V(Unet), mode = "all", loops = TRUE, normalized = FALSE)
...
```{r weight, echo = TRUE, include = TRUE, exercise = TRUE}
library(ggraph)
ggraph::ggraph(net, layout = 'stress') + 
...
```{r strength, echo = TRUE, include = TRUE, exercise = TRUE}
igraph::strength(net) 
```
...
```{r subgraph, echo = TRUE, include = TRUE, exercise = TRUE}

sub <- igraph::induced_subgraph(net, c('s01','s02'))
# sub <- igraph::induced_subgraph(net, 1:7)
...
```{r cliques, echo = TRUE, include = TRUE, exercise = TRUE }
igraph::cliques(net, min = 4, max = 6) ## all the cliques beween min e max n nodes

# igraph::largest_cliques(net)
# 
# igraph::max_cliques(net)
```
...
```{r assort_degree, echo = TRUE, include = TRUE, exercise = TRUE }
# random graph
...
```{r assort, echo = TRUE, include = TRUE, exercise = TRUE}

assortativity(net, V(net)$audience.size, directed=T)
