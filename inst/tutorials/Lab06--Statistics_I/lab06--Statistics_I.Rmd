...
likes another at timepoint 3 depends positively on how much he liked him at 
the previous time points and how much he is liked by the other monk, and 
...
follows (allow for 20 seconds or so for this to finish running, 
...
data for that example: it offers valued networks that we can use as  
appropriate dependent variables. Note that MRQAP only requires the dependent 
variable to be valued, the explanatory networks can be binary, valued, or 
combinations. 
...
<br>We calculate this variable by taking 
...
In the box below, compute the number of present ties and the number of 
absent ties in `flobus_network`. Also, determine the ratio of the two. 
