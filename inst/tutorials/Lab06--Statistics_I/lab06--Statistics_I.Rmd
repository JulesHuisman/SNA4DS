...
* [Next](#QAP), you will learn how to compare two networks 

* Finally, you will learn how to regress a network on several others, using 
[linear](#MRQAP) and [loglinear](#netlogit) models.

Awesomeness. So, let's get started. Let's travel back in time to Italy 600 years.
<br><br><br><br>
...
such as loans, credits and joint partnerships) and __marriage alliances__ 
(`flomarriage`).
...
closeness, and degree. But, __first__, look again at the plot you made aboven 
...
So, the only thing we need is a set of networks to test our network against. 
...
collecting them by hand. The advantage is that we now ensure that the networks 
are comparable to our own networks, and we can define what "comparable"
means ourselves! 
...
* Calculate a global measure on some network (e.g., betweenness centralization)

* Generate a large number of networks that a comparable to the original network and calculate the global measure for each

* Compare the value of the measure from your original network with those of the simulated networks

That's it. 

Through this procedure we end up with the relevant *empirical distribution* of 
the measure we are interested in. As soon as we know the distribution of some 
...
comparable, but the three most common are these: 
...
Note: this is a so-called Erdos graph, explained in a lecture by Claudia.

We don't care about these networks per se, but about their transitivity. This
is calculated in `sna` as follows:
...
There we have three networks, all with the desired properties of having 16 
vertices and 20 undirected edges.

Let's get the transitivities of three networks in one line of code:
...
irrespective of the number you specify in the previous code block. If you want 
to see the results of increasing the number of draws, copy the code for drawing 
the graph to the previous block, so it draws the graph based on whatever 
number of transitivities you calculate there).
...
You will find that about 35 percent of all networks with 16 vertices and 20 
...
Mmm, that seems quite high, wouldn't you say? 

Calculate the *p*-value for transtivity in this network:

```{r trans2, exercise = TRUE, exercise.setup = "nwgen5"}
...
```{r trans2-check}
gradethis::grade_code(correct = "Oohoo, this is how we do it!")
```
...
There are two functions in `sna` that generate CUGs and then calculates the 
*p*-value of the statistic we want to test. The best implemented function is 
`sna::cug.test`. This function allows you to pick one of the following 
...
*edges* (for edge count, hence density), or *dyad.census*: 
...
I'll give you more examples later, so it will be very clear to you how that works.
...
sna::cug.test(flomar_network, FUN = sna::gtrans, cmode = "edges", 
...
larger than that which you would expect in a random network of equal size and 
density.

Now, calculate this for the business network
...
sna::cug.test(flobus_network, FUN = sna::gtrans, cmode = "edges", reps = 1000, FUN.args = list(mode = "graph"))
```
...
As expected, the *p*-value is, rounded, 0. The transitivity of the business 
...
cug_trans <- sna::cug.test(flobus_network, FUN = sna::gtrans, cmode = "edges", reps = 1000, FUN.args = list(mode = "graph"))
...
of a single network 
is high/low. We now move to the situation where you have two networks you 
would like to compare. We will limit our treatment in this tutorial to the 
case where you want to compare networks that you have collected 
with the same vertices, such as a friendship network and advice network among the
...
This is a special case of the more general "graph comparison problem." 

The most common linear subspace method for this purpose is the so-called 
*Quadratic Assignment Procedure* (QAP). QAP is a method that was originally 
developed for somewhat other purposes, but it has been extended for use with 
social networks. As a result, you may find references to QAP in literature that
does not relate to networks. So you know.

QAP is similar to CUG, in that it uses 
statistical simulation in
order to generate a distribution of hypothetical networks. 
However, with QAP we do not generate a series of graphs out-of-the-blue, but we now 
control for all purely structural properties of the graphs being compared 
themselves.

How does QAP control for things like size, density, degree, and all further 
structure that is inherent in the two networks? Well, it does so by permuting 
the networks. In its most basic form, this works as follows:

1. Take one of the two networks (say, the first)

2. Randomly permute the order of the vertices in the network. If your original 
network consisted of 5 vertices (ordered as 1, 2, 3, 4, 5), you would now 
reshuffle, your vertices, for example, as 1, 5, 2, 4, 3. You then create the 
adjacency matrix by keeping the first row, then place row 5, then rows 2, 4, and 3. You then also reorder the columns that way, so you start with column 1, then 5, then 
2, 4, and 3. You thus end up with a network that has the same number of vertices,
and maintains other important characteristics.

3. Calculate the correlation between this shuffled network with the non-shuffled
network 2.
...
between the original networks to this distribution. This again yields a *p*-value.
...
That is a moderate correlation. Or is it large? Let's run it:

```{r remedy024, exercise = TRUE}
floCor <- sna::qaptest(list(flobus_network, flomar_network), 
                       sna::gcor, g1 = 1, g2 = 2, reps = 1000)
floCor
```
...
The QAP looks at the relation between to networks. Let's exend this to 
multiple networks and model one network as a linear function of a bunch of 
others. 

For this method, we are going to make use of a different dataset. 
...
are regressed. So, if we wanted to test the hypothesis that how much a monk 
likes another at time 3 depends positively on how much he liked them at 
the previous time points, at how much he is liked by the other monk, and 
negatively on how much desesteem the monk feels for the other.

These monks all rated each other, and are rated by one another and it is 
obvious that our observations of liking and desesteem are not independent 
...
definitely mutual influence going on. 
...
not give us correct confidence intervals because the underlying inferential 
...
Y network. Another is to permute the X networks (a single one or jointly). 
Yet another is to permute the network of disturbances. 
...
that is fine, as long as you remember that the *qapspp* makes interpretation 
of the regression coefficients quiet straightforward. Let's say you get the 
following results from a *qapspp* analysis:

$\hat{Y} = 2 + 3*X_1 + 4*X_2$

In this case, you can conclude that the presence of an edge in $X_1$ increases 
the value of an edge in $Y$ with 3 compared to when that edge in $X_1$ is 
absent, controling for the effect of $X_2$ on $Y$. This is identical to the OLS 
case.
With some of the other QAP approaches, interpretation of coefficients changes, 
and I'd rather make life straightforward for us. So, all you need to remember 
is: use the *qapspp* algorithm and interpret the results as you would in 
a normal regression model (and be assured that your *p*-values are correct).


The function to run QAP regression is `sna::netlm`. We run the model as 
follows (allow for up to 10 second for this to finish running, depending on 
your machine): 
...
* in the `×` you include a list with the explanatory networks. 
...
time 3). That is why the transposed matrix is included here. 
...
So, what did we find? Well, liking at times 1 and 2 each statistically 
significantly positively affect liking at time 3. The most recent liking 
...
that person. Even monks seem to like more those others whom they experience 
being liked by.
...
but it could certainly be expected that increasing desesteem would lower how 
much they are liked. The coefficient is indeed negative, but the effect is not 
at all statistically significant.
...
*Sampson_praise*, and *Sampson_esteem*. Below, run the model (as a single model, with 
all effects in the same regression) and interpret the findings. 
...
you have a dependent network that is valued. But it is not appropriate when 
you have a binary dependent network.
...
same statistical reasons.

In a standard regression modeling context, you would use a logistic model 
(or a version of it) to model binary dependent variables. Again, same thing 
here. Of course, we do need to take care of the network dependency between the 
edges, but the model we will use is otherwise similar to the logistic 
regression model you are already familiar with. 
...
between families of comparable wealth. We calculate this variable by taking 
the absolute value of the difference in wealth between the two families. The 
higher this number, the higher the difference in wealth between them.
...
that measures each family's net wealth in thousands of lira (this is 
in Renaissance times, remember?):
...
We now run our analysis, in order to test our hypotheses. Since we have a 
binary dependent variable, the function is now called `sna::netlogit`, but 
further works in the exact same way as the `sna::netlm` function.
...
odds ratios by taking their exponent. To interpret the odds ratios, think of 
...
is about 13 times more likely to form in the presence of a marriage tie, 
than in the absence of a marriage tie. (you may get a slightly different 
number, depending on how many replications you run).

If you are interested in betting, then think of this as the odds of forming 
a business tie being 13:1 in the presence of a marriage tie. I think you can 
safely bet on a business tie

Wealth difference is not statistically significant, so we interpret wealth 
difference as not explaining business ties.
...
business ties are explained very poorly. When you perform an actual analysis, 
you would then try to find other explanatory factors that explain the 
dependent network better. 
