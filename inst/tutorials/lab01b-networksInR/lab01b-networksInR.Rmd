...
```{r ge_asadj, exercise = TRUE, exercise.setup = "load_data"}
...
```{r load_advice_igraph, include = FALSE}
advice_igraph <- SNA4DS:::advice_igraph
```

Let's look at the network a little bit.
```{r igraph_nw, exercise.setup = "load_advice_igraph", exercise = TRUE}
...
```{r advice_network_inlezen, include = FALSE}
advice_network <- SNA4DS:::advice_network
```


```{r nw_check, exercise.setup = "advice_network_inlezen", exercise = TRUE}
# you can see the adjacency matrix as follows:
...
```{r ig_check, exercise = TRUE, exercise.setup = "advice_network_inlezen"}
...
```{r check_network, exercise = TRUE, exercise.setup = "load_advice_igraph"}
# convert it
