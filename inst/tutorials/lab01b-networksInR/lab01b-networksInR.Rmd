...
```{r g1, include = TRUE, echo = TRUE, exercise = TRUE, cache=TRUE}
...
```{r g2, echo = TRUE, exercise = TRUE, cache=TRUE}
...
```{r g2_neighbors, exercise = TRUE, dependson="g2"}
igraph::V(g2)
igraph::E(g2)
...
```{r cheyenne, echo = FALSE, cache=TRUE}
data("emon", package = "network")
Cheyenne <- emon$Cheyenne
```
...
```{r grade_emon_help1, exercise = TRUE}
...
Now, below look at the object `Cheyenne`. Print it, get a summary, and make a simple plot.

```{r grade_emon_print, exercise = TRUE}
...
```{r cheyenne-attrs, exercise = TRUE}
...
```{r nw_maken_data, cache = TRUE}
advice_data_frame <- read.table("advice.txt", header = TRUE)
...
```{r head, exercise = TRUE, dependson = "nw_maken_data"}
...
You can view your data in a spreadsheet-like window, using `fix()`. 

> Note that this spreadsheet-like window may open BEHIND your current window, so 
make sure to look there if you don't see it pop up. Also note that you need to 
close the spreadsheet window, before R continues. So if you are wondering why R 
is not responding, you should realize you forgot to close the spreadsheet.

```{r fix, exercise = TRUE, dependson = "nw_maken_data"}
...
```{r advice_igraph, exercise = TRUE, dependson = "nw_maken_data"}
...
```{r igraph_nw, dependson = "advice_igraph", exercise = TRUE}
...
```{r network_nw, cache = TRUE, exercise = TRUE}
advice_data_frame <- read.table("advice.txt", header = TRUE)
attributes <- read.csv('attributes.csv', header = TRUE)
...
```{r nw_check, dependson = "network_nw", exercise = TRUE}
# you can see the adjacency matrix as follows:
...
```{r advice_nws, echo = FALSE, cache = TRUE}
load("advice_nws.RData")
```


### From `network` to `igraph`
```{r to_network, dependson = "advice_nws", exercise = TRUE, cache = TRUE}
class(advice_network)
```


Convert it to a `network` object.

```{r ig_check, exercise = TRUE, dependson = "to_network"}
...
```{r to_igraph, dependson = "advice_nws"}
class(advice_igraph)
```

Now convert it to `igraph`

```{r check_network, exercise = TRUE, dependson = "to_igraph"}
# convert it
