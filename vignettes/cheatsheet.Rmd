...
This algorithm can be plotted as a dendrogram
```{r}
...
The function `cluster_louvain` implements the multi-level modularity optimization algorithm for finding community structure, see references below. It is based on the modularity measure and a hierarchial approach. 
...
The formula require the specification of a network dependent variable, 
and a list of terms. 

Terms can be classified in three main ways

* Dyadic independent and dyadic dependent terms: We encounter the first one when
the probability of edge formation is related to nodes properties or attributes; 
we encounter the second when the probability of edges formation depends on other existing edges. 

* Structural and nodal attributes terms: The first kind provide tools to understand
the structure of the network per se; the second kind provide tools to explain how nodal attributes might have influenced the formation of edges. 
...
* `edges` Probability that the observed edges are non-random. Introduces one statistics to the model. Directed and Undirected networks

* `density` Probability that the observed density is non random. Introduces one statistics to the model. Directed and Undirected networks

* `sender` Probability that a specific node, compared to a baseline one is sending out non random edges (different from the behavior of the same node in a random distribution). Introduces to the model as many statistics as the number of nodes minus one. Directed Networks only. 

* `receiver` Probability that a specific node, compared to a baseline one is receiving non random edges (different from the behavior of the same node in a random distribution). Introduces to the model as many statistics as the number of nodes minus one. Directed Networks only. 

### Most popular structural - dyadic dependent terms

* `mutual` Probability that the observed reciprocated ties are non random. Introduces one statistics to the model. Directed networks only.

* `asymmetric` Probability that the observed non reciprocated ties are non random. Introduces one statistics to the model. Directed networks only.

* `triangles` Probability that the observed triangles are non random. Introduces one statistics to the model. Directed and Undirected networks. In the case of directed network measures “transitive triple” and “cyclic triple”, so triangle equals to `ttriple` plus `ctriple`. 

* `gwesp(decay=0.25, fixed=FALSE)` Geometrically weighted edgewise shared partner distribution. It can be used in place of triangles to improve convergence. decay parameter, which should be non-negative. The value supplied for this parameter may be fixed (if `fixed=TRUE`), or it may be used instead as the starting value for the estimation of decay in a curved exponential family model (when `fixed=FALSE`, the default) (see Hunter and Handcock, 2006). This term can be used with directed and undirected networks. For directed networks, only outgoing two-path (`“OTP”`) shared partners are counted.  
...
* `triadcensus` Probability that the sixteen categories in the categorization of Davis and Leinhardt (1972) observed in the network and are non random. Introduces 16 statistics to the model. Directed networks only.

* `balance` Probability that type 102 or 300 in the categorization of Davis and Leinhardt (1972) -balanced triads- observed in the network are  non random. Introduces one statistics to the model. Directed networks only.

* `transitive` Probability that type 120D, 030T, 120U, or 300 in the categorization of Davis and Leinhardt (1972) -transitive triads- observed in the network are  non random. Introduces one statistics to the model. Directed networks only.

* `intransitive` Probability that type 111D, 201, 111U, 021C, or 030C in the categorization of Davis and Leinhardt (1972) -intransitive triads- observed in the network are  non random. Introduces one statistics to the model. Directed networks only.

* `degree(n)`, `idegree(n)`, `odegree(n)` Probability that nodes with a specified degree are non random. Introduces one statistics to the model. Directed and Undirected networks, with possibility of `in` and `out` specifications for Directed networks.

* `kstar(n)`, `istar(n)`, `ostar(n)` Probability that stars connecting the specified number of nodes are non random. Introduces one statistics to the model. Directed and Undirected networks, with possibility of `in` and `out` specifications for Directed networks.

* `cycle(n)` Probability that cycles with a specified number of nodes are non random. Introduces one statistics to the model. Directed and Undirected networks.


### Most popular nodal covariate terms

* `nodecov`, `nodeicov`, `nodeocov` Numeric or Integer attributes. Probability that the attribute values influence edges formation (same as in a logit model) so that it is non-random under that condition. Introduces one statistics to the model. Directed and Undirected networks, with possibility of `in` and `out` specifications for Directed networks. Dyadic independent.

* `nodefactor`, `nodeifactor`, `nodeofactor` Categorical attributes. Probability that nodes characterized by a specific category form more ties so that tie formation is non random under that condition. Introduces to the model a number of statistics equal to the number of categories minus one. Directed and Undirected networks, with possibility of `in` and `out` specifications for Directed networks. Dyadic independent.

* `absdiff` Numeric or Integer attributes. Probability that common features measured in terms of distance similarity influence edge formation so that edge formation is non random under that condition. Introduces one statistics to the model. Directed and Undirected networks. Dyadic independent.

* `nodematch` Categorical attributes. Probability that nodes characterized by a specific category belonging to a certain attribute form ties with other node characterized by the same category, so that tie formation under that condition is non random. Introduces to the model as many statistics as the number of categories. Directed and Undirected networks. Dyadic independent.

* `edgecov` Matrix attribute. Probability that the ties formed in another context influence tie formation in the context of the current model so that tie formation under that circustances is non random. Introduces one statistics to the model. Directed and Undirected networks. Dyadic dependent.

* `nodemix` Categorical attributes. Probability that nodes denoted by different categories of an attribute form ties so that tie formation under these circumstances in non random. Introduces as many statistics as the number of combinations between each two categories. Directed and Undirected networks. Dyadic independent.
...
* Check attribute (numeric, integer, categorical, ordinal)
...
This last one provides the number of cases under the assumptions of each term.
